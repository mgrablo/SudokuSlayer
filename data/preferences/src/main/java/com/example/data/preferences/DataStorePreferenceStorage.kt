...
internal class DataStorePreferenceStorage(
	...
	override suspend fun <T> get(key: PreferenceStorage.Key<T>): T? = getAsFlow(key).firstOrNull() ?: key.defaultValue

	override suspend fun <T> set(
		key: PreferenceStorage.Key<T>,
		value: T?,
	) {
	...
	private fun <T> getDataStoreKey(key: PreferenceStorage.Key<T>): Preferences.Key<T> =
		when (key) {
