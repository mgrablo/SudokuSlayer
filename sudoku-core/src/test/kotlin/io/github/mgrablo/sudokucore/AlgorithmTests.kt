...
class AlgorithmTests {
	...
	inner class MatrixConversionTests {
		...
		fun `should convert simple boolean matrix to DLX structure`() {
			...
			assertEquals(
				"H0",
				(root.right as HeaderNode).name,
				"First column header should be 'H0'",
			)
			assertEquals(
				"H1",
				(root.right.right as HeaderNode).name,
				"Second column header should be 'H1'",
			)
			assertEquals(
				"H2",
			...
			assertEquals(
				2,
				(root.right.right as HeaderNode).numOfNodes,
				"Column 1 should have 2 nodes",
			)
			assertEquals(
				0,
