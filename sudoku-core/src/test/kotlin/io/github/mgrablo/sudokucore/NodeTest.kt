...
class NodeTest {
	...
	fun `insertRight should insert node to the right`() {
		...
		assertEquals(
			newNode,
			secondNode.left,
			"First node should be to the left of the second node",
		)
	...
	fun `removeLeftRight should remove node from horizontal list`() {
		...
		assertEquals(
			firstNode,
			thirdNode.left,
			"First node should be to the left of the third node",
		)
		assertEquals(
			thirdNode,
			firstNode.right,
			"Third node should be to the right of the first node",
		)
	}
